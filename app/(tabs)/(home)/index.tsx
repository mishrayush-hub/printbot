      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30, paddingHorizontal: 10, paddingTop: 15 }}>
        ...
        <View className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-4 border rounded-lg ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          <View className={`border-2 border-dashed ${isDarkMode ? 'border-gray-600' : 'border-gray-300'} rounded-lg p-6 items-center justify-center ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
            ...
            <TouchableOpacity
              className="max-w-[330px] min-h-[40px] py-2 px-3 bg-[#008cff] rounded-lg items-center justify-center"
                  ...
                  <View key={index} className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} p-4 rounded-lg mb-3 border`}>
                    ...
                    <View className="flex-row items-start justify-between mb-3">
                      <View className="flex-row items-center flex-1">
                        <View className="w-8 h-8 bg-white rounded items-center justify-center mr-3">
                      ...
                      {/* Status Badge */}
                      <View className={`px-2 py-1 rounded-md ${paid ? 'bg-green-100 dark:bg-green-900' :
                    ...
                    <View className="flex-row justify-end">
                      {!uploaded && !paid && (
                        <TouchableOpacity
                          className="bg-red-500 max-w-[200px] min-h-[35px] px-4 items-center justify-center rounded-[8px] mr-2"
                          onPress={() => {
                            Alert.alert(
                              "Remove File",
                        ...
                        >
                          <Text className="text-white text-base font-medium">Remove</Text>
                        </TouchableOpacity>
                      )}

                      {!uploaded && !paid && (
                        <TouchableOpacity
                          className="max-w-[200px] min-h-[35px] px-4 bg-[#008cff] rounded-lg items-center justify-center"
                          onPress={() => {
                            Alert.alert(
                              "Upload File",
                        ...
                        >
                          <Text className="text-white text-base font-medium">Upload File</Text>
                        </TouchableOpacity>
                      )}
            ...
            <View className="mb-4">
              <View className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-4 rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                ...
                {uploadedFiles.length > 0 && uploaded && !paid && (
                  <TouchableOpacity
                    className="min-h-[40px] mt-4 bg-[#008cff] rounded-lg items-center justify-center"
